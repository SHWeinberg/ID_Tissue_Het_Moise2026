function [ts, phi_mat, Gmat, Smat, Iall_mat, tup, trepol, p, cv_est, store_out] =  ...
    generic_IDnano_epc_v6(ionic_fun_name, p, Ncell, Mdisc, Gb_mat, Gc_array,...
...
% post-junctional localization, and the second Mdisc rows are the
% pre-junctional localization.
%
% If loc_mat is a 3D array (with Ncell "slices"), then each "slice" is the   
% localization values for each cell
%
% IDarea_vec: ID membrane patch area (um^2), Mdisc x 1.  Assumed to be
% symmetric (both sides of cleft) and same for all cells.  If IDarea_vec is
% Mdisc x 2*Ncell, then each column represents the areas for the post- and
% pre-junctional membranes
%
...
%
% Vol_cleft_vec: cleft compartment volume vector (um^3), Mdisc x 1 (or
% Mdisc x Ncell-1 where each column is the cleft compartment volume vector
% for each junction)
%
% clamp_flag: flag to clamp cleft concentrations (1 = clamped), 1 x Nextra (for
% Na+, K+, Ca2+, and can include A-)
%
% zvec: charge valence, 1 x Nextra (for Na+, K+, Ca2+), can also include A-,
% where Nextra is number of extracellular ions tracked in cleft
%
% dtparams structure:
...
%
% New in v6:
% 1. generalized the extracellular ion tracking to include anions in the
% cleft space
% 2. localization can differ between different junctions.  If loc_mat has
% Ncell "slices" (i.e., 3rd dimension), then each slice defines the
% localization for that cell. Be careful about defining
% spatially heterogeneous localization values. If the localization is
% defined symmetrically, i.e., the localization is the for both
% pre-junction and post-junction membranes, then loc_mat must defined such
% that loc_matPre(:,:,i) = loc_matPost(:,:,i+1)
% 3. IDarea_vec and Vol_cleft_vec can have additional dimensions to
% represent spatial variation between cells and junctions, respectively
%
% New in v5:
...
% 3. includes parameters, calculations for intracellular diffusion (i.e.,
% between axial and ID compartments, and between adjacent ID compartments)
...
if nargin == 0
    ...
    if 0
        ...
        [Gb_mat, Gc_array] = calc_grid_Gb_Gc(w_vec, p_ext, f_bulk, f_disc);
    end
...
[tmp_a, ~, tmp_c] = size(loc_mat);
if tmp_c == 1
    loc_mat = repmat(loc_mat,1, 1, Ncell);
end
...
if tmp_a == Mdisc
    ...
    loc_matPost = loc_mat(1:Mdisc,:,:);
    loc_matPre = loc_mat(Mdisc+1:end,:,:);
end

[~, tmp_b] = size(IDarea_vec);
if tmp_b == 1
    IDarea_vec = repmat(IDarea_vec, 1, 2*Ncell);
end
...
Nextra = length(zvec);  % number of extracellular ions tracked
...
Aax = 2*pi*r*L; % patch surface area, um^2
% Ad = pi*r^2;    % disc surface area, um^2
Ad = sum(IDarea_vec,1); % disc surface area, um^2, 1 x 2*Ncell vector
Atot = zeros(1, Ncell);
if length(Ad)>1
    for i = 1:Ncell
    ...
    Atot = (2*Ad + Aax)*ones(1, Ncell);  % total surface area, um^2
end
Cax = Cm*Aax;    % axial capacitance, uF
IDarea_vecPost = IDarea_vec(:,1:2:end); % post-junctional ID surface area, um^2, Mdisc x Ncell matrix
IDarea_vecPre = IDarea_vec(:,2:2:end); % pre-junctional ID surface area, um^2, Mdisc x Ncell matrix

% Cdisc = Cm*IDarea_vec;  % cleft capacitance, uF, Mdisc x 1 vector
% CL = repmat(Cdisc',Ncell, 1); % uF, left side (post-junctional) ID patch capacitance, Ncell x Mdisc matrix
% CR = repmat(Cdisc',Ncell, 1); % uF, right side (pre-junctional) ID patch capacitance, Ncell x Mdisc matrix

CL = Cm*IDarea_vecPost'; % uF, left side (post-junctional) ID patch capacitance, Ncell x Mdisc matrix
CR = Cm*IDarea_vecPre'; % uF, right side (pre-junctional) ID patch capacitance, Ncell x Mdisc matrix

p.Ctot = Cm*constructCell2Patches(Ncell, Mdisc, Atot);   % total cell capacitance, uF, 1 x Npatches vector
...
[~,tmp_b] = size(Vol_cleft_vec);
if tmp_b == 1
    Vol_cleft_all = repmat(Vol_cleft_vec,  Ncell-1, 1);
else
    Vol_cleft_all = Vol_cleft_vec(:);
end
...
if 0
    ...
    Ctot_ave = (p.Ctot(1:end-1) + p.Ctot(2:end))'/2;
    Rgap = R./Ctot_ave;  % k-ohms ( = ms/uF)
end
...
loc_ax = zeros(Ncurrents, Ncell);
for i = 1:Ncell
    loc_ax(:,i) = 1-sum(loc_matPre(:,:,i),1)-sum(loc_matPost(:,:,i),1);  % localization on axial membrane
end
f_I = zeros(Ncurrents, Npatches);
for q = 1:Ncurrents
    if ~iscell(scaleI)
        for i = 1:Ncell
            f_I(q,(i-1)*(2*Mdisc+1)+1) = scaleI(q)*loc_ax(q,i);
            for j = 1:Mdisc
                if i>=2 % post-junctional membrane
                    f_I(q,(i-2)*(2*Mdisc+1)+Mdisc+j+1) = scaleI(q)*loc_matPost(j,q,i);
                end
                
                if i<=Ncell-1  % pre-junctional membrane
                    f_I(q,(i-1)*(2*Mdisc+1)+j+1) =  scaleI(q)*loc_matPre(j,q,i);
                end
            end
        end
    else
        if length(scaleI{q})==1
            ...
            for i = 1:Ncell
                f_I(q,(i-1)*(2*Mdisc+1)+1) = scaleI{q}(i)*loc_ax(q,i);
                for j = 1:Mdisc
                    if i>=2
                        f_I(q,(i-2)*(2*Mdisc+1)+Mdisc+j+1) = scaleI{q}(i)*loc_matPost(j,q,i);
                    end
                    
                    if i<=Ncell-1
                        f_I(q,(i-1)*(2*Mdisc+1)+j+1) =  scaleI{q}(i)*loc_matPre(j,q,i);
                    end
                end
            end
        end
...
S = zeros(Nextra*Ncleft_comp, 1);               % cleft ion concentrations
Sb = p.cleft_conc';

% initial conditions

if ~isfield(p,'continue')
    p.continue.flag = 0;
end
if ~p.continue.flag
    ...
    cleft_clamp = []; 
    for q = 1:Nextra
        cleft_clamp = [cleft_clamp; clamp_flag(q)*ones(Ncleft_comp,1)];
        S(1+(q-1)*Ncleft_comp:q*Ncleft_comp) = Sb(q);
    end
...
if 0
    ...
    L_IDID = sqrt(mean(Ad)/Mdisc); % characteristic length of intra-ID intracellular diffusion
...
%     else
%         ibefore = sign(t_store-(round(ti-dt,5)));
%         iafter = sign(t_store-ti);
%         if any(ibefore.*iafter<0)
%             1; % 
%         end
    end
...
% simple conduction velocity calculation (deltaX/deltaT)
